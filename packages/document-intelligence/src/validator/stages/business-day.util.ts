 ...
 *
 * Canonical implementation — shared by the API (event seeding, reminders,
 * welcome-email timeline) and the Final Negotiated Terms resolver in this
 * package, so a contingency deadline shown in the UI is always computed the
 * exact same way as the one scheduled/emailed by the backend.
 *
 * Contingency day-counts themselves (loan/appraisal/inspection/disclosures)
 * are counted in calendar days, per C.A.R. RPA's definition of "Day"/"Days"
 * (Section 25) — only the FINAL deadline is rolled forward to the next
 * business day when it lands on a weekend or holiday. EMD/initial-deposit
 * terms phrased as "N business days after Acceptance" use `addBusinessDays`
 * instead, which skips weekends/holidays in the counting itself.
 */
