...
const ZONE_LABEL = 'the RPA page 9 footer';
const PAGE_LABEL = 'RPA page 9';

const RPA_PAGE_9_SCHEMA = {
  page_number: 9,

  page_validation: {
    is_rpa_page_9:
      '<boolean — true ONLY if the footer identifies the page as "RPA PAGE 9 OF 17">',
    form_revision:
  ...
  execution_review: {
    ...
    full_signatures: {
      full_signature_fields_present_on_page:
        '<boolean — normally false because RPA page 9 contains footer initials only>',
      ...
      signature_review_status:
        '<"not_applicable_on_page_9"|"unexpected_signature_field_detected">',
  ...
  page_9_footer_initials: initialsZoneSchema(ZONE_LABEL),
};
...
SCHEMA:
${JSON.stringify(RPA_PAGE_9_SCHEMA, null, 2)}
`;

export const rpaPage9ExecutionReview: PageDefinition = {
  pageNumber: 9,
  systemPrompt: SYSTEM_PROMPT,
  userPrompt: `

Analyze logical RPA page 9 for missing Buyer and Seller initials in the page footer.
...
Do not report missing full signatures because full Buyer and Seller signatures are not required on RPA page 9.
