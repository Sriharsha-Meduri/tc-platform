...
const ZONE_LABEL = 'the RPA page 7 footer';
const PAGE_LABEL = 'RPA page 7';

const RPA_PAGE_7_SCHEMA = {
  page_number: 7,

  page_validation: {
    is_rpa_page_7:
      '<boolean — true ONLY if the footer identifies the page as "RPA PAGE 7 OF 17">',
    form_revision:
  ...
  execution_review: {
    ...
    full_signatures: {
      full_signature_fields_present_on_page:
        '<boolean — normally false because RPA page 7 contains initials only>',
      ...
      signature_review_status:
        '<"not_applicable_on_page_7"|"unexpected_signature_field_detected">',
  ...
  page_7_footer_initials: initialsZoneSchema(ZONE_LABEL),
};
...
SCHEMA:
${JSON.stringify(RPA_PAGE_7_SCHEMA, null, 2)}
`;

export const rpaPage7ExecutionReview: PageDefinition = {
  pageNumber: 7,
  systemPrompt: SYSTEM_PROMPT,
  userPrompt: `
Analyze logical RPA page 7 for missing Buyer and Seller initials.
...
are not required on RPA page 7.
