...
import { TitleContactsService, CreateTitleContactDto, UpdateTitleContactDto } from './title-contacts.service';

@Controller('title-contacts')
export class TitleContactsController {
  constructor(private readonly service: TitleContactsService) {}
  ...
  @Post()
  create(@Request() req: { user?: { userId?: string } }, @Body() dto: CreateTitleContactDto) {
    return this.service.create(this.getUserId(req), dto);
  }

  @Patch(':id')
  update(@Request() req: { user?: { userId?: string } }, @Param('id') id: string, @Body() dto: UpdateTitleContactDto) {
